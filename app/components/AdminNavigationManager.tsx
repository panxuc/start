...
export default function AdminNavigationManager() {
  ...
  function saveEditingLink() {
    ...
    updateDraft((draft) => {
      const currentLinks = [...(draft[activeCategory] || [])];
      if (!currentLinks[editingLinkIndex]) {
        return draft;
      }
      ...
      draft[activeCategory] = currentLinks;
      return draft;
    });
  ...
  }, [categories, isDirty, addName, addUrl, activeCategory, editingLinkIndex, editingName, editingUrl, navSnapshot, siteSettings, siteSnapshot, token]);
              ...
              {links.map((link, index) => (
                ...
                >
                  {editingLinkIndex === index ? (
                    <div className="min-w-0 grid grid-cols-1 gap-1.5">
                      <input
                        ...
                        className="rounded-md border border-zinc-300 dark:border-zinc-600 bg-zinc-50 dark:bg-zinc-800 px-2 py-1.5 text-sm outline-none"
                        placeholder="名称"
                      />
                      <input
                        value={editingUrl}
                        onChange={(event) => setEditingUrl(event.target.value)}
                        className="rounded-md border border-zinc-300 dark:border-zinc-600 bg-zinc-50 dark:bg-zinc-800 px-2 py-1.5 text-xs outline-none"
                        placeholder="https://example.com"
                      />
                    </div>
                  ) : (
                  ...
                  <div className="flex flex-wrap items-center gap-1">
                    ...
                    <button
                      onClick={() =>
                        editingLinkIndex === index ? saveEditingLink() : startEditingLink(index)
                      }
                      className="rounded-md border border-blue-300 dark:border-blue-800 text-blue-700 dark:text-blue-300 px-2 py-1 text-xs"
                    >
                      {editingLinkIndex === index ? "保存" : "编辑"}
                    </button>
                    {editingLinkIndex === index && (
                      <button
