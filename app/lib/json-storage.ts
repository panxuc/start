...
function resolveFromProjectRoot(...segments: string[]): string {
  return path.join(/*turbopackIgnore: true*/ process.cwd(), ...segments);
}

function resolveIgnoredFromProjectRoot(...segments: string[]): string {
  return path.join(/*turbopackIgnore: true*/ process.cwd(), ...segments);
}
...
export function resolveJsonPath(rawPath: string | undefined, fallbackName: string): string {
  ...
  if (configuredPath) {
    return path.isAbsolute(configuredPath)
      ? configuredPath
      : resolveIgnoredFromProjectRoot(configuredPath);
  }

  const dataDir = process.env[START_DATA_DIR_ENV_KEY]?.trim();
  if (!dataDir) {
    return resolveFromProjectRoot("data", fallbackName);
  }

  return path.isAbsolute(dataDir)
    ? path.join(dataDir, fallbackName)
    : resolveIgnoredFromProjectRoot(dataDir, fallbackName);
}
...
export async function writeJsonFile(filePath: string, value: unknown): Promise<void> {
  ...
  await writeFile(/*turbopackIgnore: true*/ filePath, `${JSON.stringify(value, null, 2)}\n`, "utf8");
}
